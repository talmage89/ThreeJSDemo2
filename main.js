...
loader.load( './assets/avatar2.glb', function ( gltf ) {
...
const moonMaterial = new THREE.MeshBasicMaterial({ 
  color: 0xffffff,
  map: moonTextureLoader.load('./assets/moon.jpeg'),
});
...
const circleTextureLoader = new THREE.TextureLoader();
const circleGeometry = new THREE.CircleGeometry(5, 40);
const circleMaterial = new THREE.MeshBasicMaterial({
  color: 0xffffff,
  map: circleTextureLoader.load('./assets/circle.png')
});
const circle = new THREE.Mesh(circleGeometry, circleMaterial);
circle.position.x = -40;
circle.position.z = 210;
circle.rotation.y = 2.5;
scene.add(circle)
...
function animate() {
  ...
  circle.rotation.x = Math.cos(time) / 4 ;
